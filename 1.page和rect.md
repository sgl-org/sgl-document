...
![0](imag/1/0.png)
翻阅SGL的库文件可知，如果你在创建一个控件的时候，给他夫对象是NULL他会**自动创建一个page**，让控键显示page上面，如果用户自己创建 `Page` 时，**须显式调用 `sgl_screen_load()` 才能显示在屏幕上**。
